...
Сначала переводим адрес в десятеричную систему. Далее переводим в двоичную строку.
После чего добавляем незначащие нули.

4) net.hosts() - ЕСЛИ СПРАШИВАЮТ ПРО КОЛ-ВО УЗЛОВ/ФИЗ.УСТРОЙСТВ/АДРЕСОВ В СЕТИ, ИСПОЛЬЗУЕМ
net.hosts() - убирает 2 широковещательных адреса в сети (первый и последний)
for i in net.hosts():
    print(i)
