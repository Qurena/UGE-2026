...
# ВСЕГДА НАВЕШИВАТЬ ?= (по возможности)
'''
...
4. sub(pattern, replacing, string) 
с помощью этой функции мы можем заменить подстроки, подходящие под паттерн на replacing в строке string. 
s = sub('[0-9]+', ' ', s) 
на выходе получаем строку, у которой вместо любой последовательности цифр пробел

5. lookahead ( (?=(регулярка) ) - допустим, нам нужно найти макс длину строки, в которой ровно 50 сочетаний символов CD. 
...
ВАЖНЫЕ НЮАНСЫ:
- если мы понимаем, что какая-то часть регулярки не накладывается, мы можем вынести ее из lookahead:
XYZ(CxD[A-D]*) -> XYZ(?=((CxD[A-D]*)))
- при этом g = i.group(1) будет считать только строку внутри lookahead. То есть при подсчёте длины мы 
будем писать len(g) + 3, тк XYZ - 3 символа.

6. Хочу найти последовательность, заключенную между цифрами:
pattern = r"([1-9]+).*\1" # .* - любая последовательность, заключенная между [1-9]+

7. Хочу найти все повторяющиеся числа:
pattern = r"([0-9]+)\1+"

8. Поиск последовательностей, начинающихся и оканчивающихся на одну четную цифру: 
pattern = r"([02468])[A-Z]+\1" - pat = '(то, что по бокам)что внутри\1' !!!!!!!!!!
4AWRPETEPW4236562FPSF2 # '4AWRPETEPW4' и '2FPSF2' подойдут

9. ПЕРВЫЙ СИМВОЛ - единственная четная цифра в последовательности и ровно 35 букв "S":
pattern = r'(?=([02468][^S02468]*([S][^S02468]*){35}))' !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

10. i1 = el.start() - индекс первого символа в подстроке, i2 = el.end() - индекс последнего символа в подстроке,

11. Ровно 28 символов Z в подстроке, нач. на K или на 9: 
pattern = '(?=([9K][^Z]*([Z][^Z]*){28}))' !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
