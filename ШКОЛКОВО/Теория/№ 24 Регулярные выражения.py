...
5. Хочу найти последовательность, заключенную между цифрами:
pattern = r"([1-9]+).*\1" # .* - любая последовательность, заключенная между [1-9]+

6. Хочу найти все повторяющиеся числа:
pattern = r"([0-9]+)\1+"

7. Поиск последовательностей, оканчивающихся на одну четную цифру: 
pattern = r"([02468])[A-Z]+\1"
4AWRPETEPW4236562FPSF2 # '4AWRPETEPW4' и '2FPSF2' подойдут

8. ПЕРВЫЙ СИМВОЛ - единственная четная цифра в последовательности и ровно 35 букв "S":
